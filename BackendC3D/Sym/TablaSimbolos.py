...
class TablaSimbolos:
    ...
    def guardarVar(self, ident, tipo, enHeap, find = True):
        if find:
            envActual = self
            while envActual != None:
                if ident in envActual.tablaS:
                    envActual.tablaS[ident] = Simbolo(ident, tipo, envActual.envAnterior == None, envActual.tablaS[ident].getPosicion(), enHeap)
        ...
        if ident in self.tablaS:
            ...
            nuevoSimb = Simbolo(ident, tipo, self.envAnterior == None, self.size, enHeap)
            self.size += 1
            self.tablaS[ident] = nuevoSimb
            TablaSimbolos.variables = self.tablaS
            return self.tablaS[ident]
